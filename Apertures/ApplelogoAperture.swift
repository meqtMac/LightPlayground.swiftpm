...
import SwiftUI
struct AppleLogoAperture: Aperture {
    let id = UUID()
    ...
    func path(in rect: CGRect) -> Path {
        ...
        path.closeSubpath()
//        let trans
        let transform = CGAffineTransform.identity.translatedBy(x: -90, y: -90)
            .scaledBy(x: rect.width / 120 * 2 * radius / size.width,
                      y: rect.height / 120 * 2 * radius / size.height)
